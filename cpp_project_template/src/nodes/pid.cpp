...
void PidNode::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    if (state_ == DriveState::DRIVE_FORWARD) {
        ...
        // PID regulácia podľa pravej steny (alebo ľavej, ak nie je pravá)
        float error = 0.0f;
        if (right_side < front_side_threshold) {
            error = 0.25f - back_right;  // cieľová vzdialenosť od pravej steny
        } else if (back_left < front_side_threshold) {
            error = back_left - 0.25f;  // ak nie je pravá, reguluj podľa ľavej
        } else {
            error = 0.0f;
        }
        ...
        if (front_left < side_threshold) speed_diff -= 1.5f;
        if (front_right < side_threshold) speed_diff += 1.5f;
        ...
        float target_yaw = full_turn
            ? turn_direction_ * (PI / 2.0f)
            : turn_direction_ * ((PI / 2.0f) - PI / 18.0f);
